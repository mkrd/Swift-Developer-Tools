	...
	var description: String
	{
		...
		for c in 0..<self.cols
		{
			...
			for r in 0..<self.rows
			{
				...
				let rcCount = strArray[r][c].characters.count
		...
		for r in 0..<self.rows
		{
			...
			for c in 0..<self.cols
			{
				let padLength = (colMaxChars[c] - strArray[r][c].characters.count + 4) / 2
				let trailLength = (colMaxChars[c] - strArray[r][c].characters.count + 1) / 2
